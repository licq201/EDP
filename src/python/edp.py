...
class EDP:
    ...
    def __init__(
        ...
        # L7 保形预测层（2025 前沿：有限样本覆盖率保证 + 分布漂移鲁棒）
        conformal_cfg = self.config.get("conformal", {})
        self.conformal_engine = ConformalEngine(
            ConformalConfig(
                alpha=conformal_cfg.get("alpha", 0.1),
                method=conformal_cfg.get("method", "aci"),
                aci_gamma=conformal_cfg.get("aci_gamma", 0.01),
            )
        )
    ...
    def add_evidence(self, evidence: list[Evidence]) -> SituationAssessment:
        ...
        更新策略（log-odds 序贯贝叶斯）：
            - 定向证据（outcome_id 非空）：对该结果做定向 log-odds 更新，
              其它结果按补集等比收缩，保持概率和为 1。
            - 非定向证据（outcome_id 为空）：视为整体方向性证据，
              按其融合概率对所有结果做加权调整。
        ...
        for e in evidence:
            ...
            sources.append(
                EvidenceSource(
                    ...
                    data={"probability": prob, "outcome_id": e.outcome_id},
        ...
        if self.probabilities:
            ...
            log_odds: dict[str, float] = {
                oid: math.log(max(p, eps) / max(1.0 - p, eps))
                for oid, p in self.probabilities.items()
            }
            weights = assessment.source_weights

            for src in sources:
                w = weights.get(src.source_id, 0.0)
                if w <= 0:
                    continue
                p_src = max(eps, min(1 - eps, src.probability))
                lo_src = math.log(p_src / (1 - p_src))

                oid = src.data.get("outcome_id")
                if oid and oid in log_odds:
                    ...
                    # 非定向证据：按结果概率加权分配到所有结果
                    total_p = sum(self.probabilities.values())
                    for oid2 in log_odds:
                        share = self.probabilities[oid2] / max(total_p, eps)
                        prior_lo2 = math.log(
                            max(self.probabilities[oid2], eps)
                            / max(1.0 - self.probabilities[oid2], eps)
                        )
                        log_odds[oid2] += w * share * (lo_src - prior_lo2)

            # 转回概率并归一化（softmax 风格，数值稳定）
            max_lo = max(log_odds.values())
            exp_lo = {oid: math.exp(lo - max_lo) for oid, lo in log_odds.items()}
            total_exp = sum(exp_lo.values())
            self.probabilities = {oid: v / total_exp for oid, v in exp_lo.items()}
    ...
    def conformal_predict(self) -> PredictionSet:
        """
        L7 保形预测：生成有覆盖率保证的预测集。

        返回一个预测集（结果 ID 列表），在有限样本下以 ≥ 1−α 的概率
        包含真实结果。ACI 模式下对分布漂移鲁棒。

        ⚠️ 覆盖率为边际保证，不针对单点。详见 conformal.py 警示。
        """
        return self.conformal_engine.predict(self.probabilities)

    def conformal_update(self, actual_outcome: str) -> dict[str, Any]:
        """用真实结果在线更新保形层（ACI/AgACI 自适应）。"""
        return self.conformal_engine.update(self.probabilities, actual_outcome)
    ...
    def analyze(
        ...
        # L7: 保形预测集（有限样本覆盖率保证）
        prediction_set = self.conformal_predict()
        ...
        summary = (
            ...
            f"预测集: {prediction_set.prediction_set}"
        )
